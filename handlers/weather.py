...
async def weather_section(call: types.CallbackQuery) -> None:
    await dp.bot.send_message(chat_id=call.message.chat.id, text=answer['input_city_reply'])
...
async def weather_in_city(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        response = answer['weather_reply'].format(weather.city, weather.info, weather.temp)
        await dp.bot.send_message(chat_id=message.chat.id, text=response, reply_markup=menu)
