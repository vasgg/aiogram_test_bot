...
async def first_code_check(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        first_code = get_currency_code(code)
...
async def second_code_check(message: types.Message, state: FSMContext) -> None:
    try:
        code = message.text.upper()
        second_code = get_currency_code(code)
        async with state.proxy() as data:
            first_code = data["first_code"]
            amount = data["amount"]
        result: ExchangeResult = await get_exchange_rate_from_message(first_code=first_code, second_code=second_code, amount=amount)
        response = answer['exchange_reply'].format(curency_code[result.first_code], curency_code[result.second_code], result.amount, result.first_code,
                                                   result.result_amount, second_code, result.first_code, result.conversion_rate, second_code)
