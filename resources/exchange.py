...
async def get_exchange_rate_from_message(first_code: str, second_code: str, amount: float) -> ExchangeResult:
    return await get_exchange_response(await get_currency_query(first_code, second_code, amount), amount)


async def get_currency_code(code: str) -> str:
...
async def get_currency_query(first_code: str, second_code: str, amount: float) -> str:
...
async def get_exchange_response(url: str, amount: float) -> json:
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
        ...
        if response.status == 200:
            return await get_exchange_rate_from_response(await response.json(), amount=amount)
    raise Exception


async def get_exchange_rate_from_response(json, amount) -> ExchangeResult:
